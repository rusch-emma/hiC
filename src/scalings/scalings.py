...
@click.option(
    '--title', '-t', 'title',
    type=str, nargs=1,
    help='Title text for the scaling plot.'
)
@click.option(
        ...
        if split_arms:
            ...
            for chrom in avail_chroms:
                ...
                sc_left, avg_trans_left = (calc_pair_freqs(
                    sc_agg[(sc_agg.chrom1 == chrom) & (sc_agg.start1 == ARM_PADDING)],
                    trans_levels,
                    show_average_trans,
                    normalized))
                ...
                dir_path = os.path.join(os.path.dirname(out_path), os.path.basename(path))
                if not os.path.exists(dir_path):
                    os.mkdir(dir_path)
                chrom_path = os.path.join(dir_path, '_'.join((chrom, os.path.basename(out_path))))
                (plot_scalings(
            ...
            if not no_cache:
                ...
                if cached is not None:
                    cis_scalings = cached['cis_scalings'] if cached['normalized'] == normalized else None
                    avg_trans = cached['avg_trans']
                
            if no_cache or cis_scalings is None or (avg_trans is None and show_average_trans):
                print(f'Computing scalings for file {idx + 1}/{len(pairs_paths)} ...', end='\r')
                ...
                print(f'Retrieved cached values for file {idx + 1}/{len(pairs_paths)}.', end='\r')
...
def compute_trans_scaling(
    ...
    avg_contacts['balanced.avg'] = avg_contacts['balanced.sum'] / avg_contacts('n_valid')
